...
class InstallMeterRequest(BaseModel):
    ...
    consumer_id: str = Field(..., description="Unique ID of the consumer.")
    meter_id: str = Field(..., description="Unique ID of the new meter.")
    install_date: datetime = Field(..., description="Timestamp of the installation.")
...
class UploadReadingRequest(BaseModel):
    """Schema for pushing meter consumption data."""
    ...
    kwh: confloat(gt=0) = Field(..., description="The consumption reading in kWh.")
...
class RechargeRequest(BaseModel):
    ...
    amount: confloat(gt=0) = Field(..., description="The payment amount.")
    transaction_ref: str = Field(..., description="Unique transaction reference from the vendor/gateway.")

class InvoiceSyncRequest(BaseModel):
    """Schema for syncing a monthly invoice (Future/Utility feature)."""
    invoice_id: conint(ge=1)
    consumer_id: str
    # CHANGE: Replace 'regex' with 'pattern'
